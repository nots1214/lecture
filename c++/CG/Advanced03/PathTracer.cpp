...
glm::vec3 PathTracer::traceRec(const Ray& ray, int recursionDepth)
{
	...
	else if (matType == Material::Diffuse_Type)
	{
		...
		phi = 2 * PI * frand();
		theta = acos(sqrt(frand()));

		const vec3 raydir = cos(phi) * cos(theta) * xLocal + sin(theta) * yLocal + sin(phi) * cos(theta) * zLocal;

		const vec3 incomingRadiance = traceRec(Ray(record.m_HitPos, raydir), recursionDepth + 1);
		const vec3& rhod = ((DiffuseMaterial*)record.m_pMaterial)->getDiffuseCoeff();
		const float rhod2 = (recursionDepth > s_MinRecursionDepth) ? std::max(rhod.x, std::max(rhod.y, rhod.z)) : 1.f;
		return rhod * incomingRadiance/rhod2;
	...
	else if (matType == Material::Blinn_Phong_Type)
	{
		...
		const vec3& rhod = ((BlinnPhongMaterial*)record.m_pMaterial)->getDiffuseCoeff();
		const vec3& rhos = ((BlinnPhongMaterial*)record.m_pMaterial)->getSpecularCoeff();
		const float rhod2 = (recursionDepth > s_MinRecursionDepth) ? std::max(rhod.x, std::max(rhod.y, rhod.z)) : 1.f;
		const float rhos2 = (recursionDepth > s_MinRecursionDepth) ? std::max(rhos.x, std::max(rhos.y, rhos.z)) : 1.f;
		float theta, phi;
		const float PI = pi<float>();
		...
		const vec3 incomingRadiance = traceRec(Ray(record.m_HitPos, raydir), recursionDepth + 1);
		const vec3 Ld = rhod * incomingRadiance;
		...
		omegah = vec2(asin(g_Camera.dir().y), atan(g_Camera.dir().z / g_Camera.dir().x));


		H = normalize(normalize(ray.getUnitDir()) + normalize(g_Camera.dir()));
		omegah = vec2(asin(H.y), atan(H.z / H.x));



		Ls = vec3(6.0, 6.0, 6.0) * rhos * incomingRadiance * dot(H, ray.getUnitDir());

		//Ls = vec3(6.0, 6.0, 6.0) * rhos * incomingRadiance * dot(omegao,omegah);



		//LdÇÕlambertÇÃÇŸÇ§,LsÇÕblinnphongÇÃÇŸÇ§ÅAç≈ëÂílÇ≈ÇÌÇÈÅA

		vec3 Ln;
		const float qsi = frand()*2;
		if (qsi < rhod2)
		{
			Ln = Ld / rhod2;
		}
		else if (qsi < rhod2 + rhos2)
		{
			Ln = Ls / rhos2;
			//Ln = vec3(0, 0, 0);
		}
